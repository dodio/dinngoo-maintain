...
| [**install-dinngoo-maintain-cron.sh**](./install-dinngoo-maintain-cron.sh) | **零参数**、`sudo`；写入 **`/etc/cron.d/dinngoo-server-maintain`**：日报（每日 02:00，统计昨天）+ 指标（约每 10 秒）+ MySQL 全量备份（每日 02:00，`maint-mysql-backup.log`） |
